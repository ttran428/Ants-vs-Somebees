    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 7
                "*** REPLACE THIS LINE ***"
                if self.ant.can_contain(insect): #if current ant can contain
                    self.ant.contain_ant(insect) #it does
                elif insect.can_contain(self.ant): #if new ant can contain
                    ant_copy = self.ant #keep copy of ant
                    self.ant = insect #set ant to be container
                    self.ant.contain_ant(ant_copy) #contains the old copy ant
                else:               
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 6A
        "*** REPLACE THIS LINE ***"
        if not self.place.ant: #if there is no ant then it is not blocked
            return False
        if self.place.ant:  #if there is an ant
            if self.place.ant.blocks_path == False:   #and if it is a ninjaAnt then it is not blocked
                return False
            return True  #else it is blocked
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        if not self.container: #if a container
            return False
        if self.ant: # and doesnt have an ant
            return False
        if other.container: #and the other ant is not a container
            return False
...
class ThrowerAnt(Ant):
    ...
    food_cost = 3
    min_range = 0
    max_range = 1000 #arbitrarily large number
    ...
    def nearest_bee(self, hive):
        ...
        "*** REPLACE THIS LINE ***"
        count = 1
        place_holder = self.place #haha get it? place holder? okay nvm
        while count <= self.min_range: #longthrower
            count += 1
            place_holder = place_holder.entrance
        count = 1
        while place_holder.entrance != hive: #until we reach the hive
            if count > self.max_range:  #shortthrower
                return None
            if place_holder.bees:            #if there are bees return the bees
                return random_or_none(place_holder.bees)
            place_holder = place_holder.entrance   #else check the next entrance
            count += 1
        return random_or_none(place_holder.bees)
        # END Problem 3B
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 6A
        for bee in list(self.place.bees):   #basically same code as FireAnt
                bee.reduce_armor(self.damage)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        ...
        if QueenAnt.queen_count == 0: #if its the true queen
            self.true_queen = True
            QueenAnt.queen_count += 1 #future queens are imposters 
        else:
    ...
    def action(self, colony):
        ...
        # BEGIN Problem 9
        def double(ant):
            if ant not in self.ants_doubled:
                ant.damage = ant.damage * 2
                self.ants_doubled.append(ant)

        if self.true_queen == False: #if it is not the true queen
            Ant.reduce_armor(self, self.armor) #die
        else:
            ScubaThrower.action(self, colony) #else be a scubaThrower
            place_holder = self.place 
            while place_holder is not None: #if the place is not none
                if type(place_holder.ant) == QueenAnt:
                    place_holder = place_holder.exit
                if place_holder.ant:
                    if place_holder.ant.container: #if its a container
                        if place_holder.ant.ant:    
                            double(place_holder.ant.ant) #double ant inside
                    double(place_holder.ant)
                place_holder = place_holder.exit #check exit
...
def make_slow(action):
    ...
    # BEGIN Problem EC
    "*** REPLACE THIS LINE ***"
    def every_other_turn_action(colony):
        if  colony.time % 2 == 0:
            return action(colony)
        else:
            pass
    return every_other_turn_action
...
def apply_effect(effect, bee, duration):
    ...
    possible_new_action = effect(bee.action)
    def new_action(colony):
        nonlocal duration
        possible_new_action(colony)
        duration -= 1
        if duration == 0:
